...
int main(int argc, char **argv)
{
    ...
    /* Desde aquí empiezas a contar 100 veces */
    int arSize = 100;
    float *host_t;
		float *device_t;
    size_t bytes = arSize*sizeof(float);

    /* Allocate memory for the times array */
    host_t = (float*)malloc(bytes);
   ...
   for (int i = 0; i < arSize; i++){
    ...
    dim3 grid((nx + block.x - 1) / block.x, 1);
		...
		SAFE_CALL(cudaMemset(), "Error setting d_MatC to 0");
